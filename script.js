...
for(const e of s){e.y+=e.v;if(e.y>c.height){e.y=0;e.x=Math.random()*c.width}e.t+=0.012+(e.layer*0.006);const alpha=0.35+0.55*((Math.sin(e.t)+1)/2);if(e.layer>0){const glow=e.layer===2?e.R*2.2:e.R*1.4;x.shadowBlur=glow;x.shadowColor=`rgba(${e.color},0.35)`;}else{x.shadowBlur=0;}x.fillStyle=`rgba(${e.color},${alpha})`;x.beginPath();x.arc(e.x,e.y,e.R,0,6.28);x.fill();x.shadowBlur=0}
